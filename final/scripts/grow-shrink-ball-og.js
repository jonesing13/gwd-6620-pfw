    ...
    collide() {
        for (var i = 0; i < balls.length; i++) {
            ...
            if (d < this.radius + balls[i].radius && this.index !== i) {
                ...
                var r = map(sin(balls[i].radius), -1, 1, 150, 255);
                var g = map(cos(balls[i].radius), -1, 1, 200, 255);
                var b = map(sin(balls[i].radius), -1, 1, 150, 255);
                fill(100, 100, b);
                this.vel.x *= -1;
                this.vel.y *= -1;
                this.radius --;
                ...
                // fill(h,s,b);
                //fill(12,65,91);
                var rElse = map(cos(balls[i].radius), -1, 1, 0, 150);
                var gElse = map(sin(balls[i].radius), -1, 1, 50, 150);
                var bElse = map(cos(balls[i].radius), -1, 1, 0, 150);
                fill(200, gElse, 200);
                this.radius += 0.01;
            }
        }
